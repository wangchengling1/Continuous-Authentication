...
def preprocess_data(data_path, window_size):
    ...
    labels = []
    #合法用户第一天数据的后一半用于测试
    file1 = os.path.join(data_path, 'w009day1-raw.csv')
    file2 = os.path.join(data_path, 'w005day1-raw.csv')
    file3 = os.path.join(data_path, 'acc_climbingup_chest.csv')
    files = [file2]
...
def create_dataloader(data, labels, batch_size=32, shuffle=True, train_ratio=0.6):
    dataset = MyDataset(data, labels, batch_size)
