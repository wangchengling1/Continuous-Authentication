...
# 使用EE（Exponential Encoding）分类器进行分类
class EETransformer(BaseEstimator, TransformerMixin):
    def __init__(self):
        pass
...
ee_classifier = Pipeline([
    ('ee_transformer', EETransformer()),
...
with torch.no_grad():
    for i, (data, _) in enumerate(train_loader):
        ...
        ee_classifier.fit(anchor_output_pca)


def add_gaussian_noise(batch_data, noise_range=0.1):
    """
    为输入的批次数据添加随机高斯噪声.

    参数:
    batch_data (torch.Tensor): 需要添加噪声的批次数据.
    noise_range (float): 噪声的最大振幅范围,默认为0.1.

    返回:
    torch.Tensor: 添加了噪声的批次数据.
    """
    noise = torch.randn_like(batch_data) * noise_range
    noisy_batch_data = batch_data + noise
    return noisy_batch_data
...
for k,(batch_data, batch_label) in enumerate(test_loader1):
    ...
    if batch_data.size(0) < 32:
        continue  # 如果数据批次小于32，跳过当前循环，继续下一次循环
    batch_data = add_gaussian_noise(batch_data)
    batch_data = batch_data.to(device)
    batch_data = batch_data.float()
    batch_data = batch_data.unsqueeze(3)
    batch_data = batch_data.reshape(32, 1, 50, 3)
    batch_data_output = model(batch_data)
    batch_data_output = batch_data_output.to('cpu')  # 将张量复制到主机内存
    batch_data_output_np = batch_data_output.detach().numpy()  # 将复制后的张量转换为 NumPy 数组
    print('batch_data_output_np', batch_data_output_np.shape)
    anchor_output_np = batch_data_output_np.reshape(32, 256)
    anchor_output_pca = pca.transform(anchor_output_np)
    predictions = ee_classifier.predict(anchor_output_pca)
